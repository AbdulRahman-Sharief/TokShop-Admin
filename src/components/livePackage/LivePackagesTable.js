...
const LivePackagesTable = ({ packages, openEdit, startDelete, openDetails }) => {
      ...
      <TableBody>
        {packages?.map(packageData => (
          <TableRow key={packageData._id}>
            ...
            <TableCell>
              <span className="text-sm">{packageData.description}</span>{" "}
            </TableCell>

            <TableCell>
              <span className="text-sm">{packageData.minutePrice}</span>{" "}
            </TableCell>
...
export default LivePackagesTable;
